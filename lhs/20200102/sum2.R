...
#     cat("", "Fixed design", "Efficiency $\\beta_e$", "Efficiency $(\\beta_e, \\beta_{et})$", "Precision $\\beta_{et}$", "Full cohort", sep=table.sep); cat("\n")
#     # cat("", "", "Adaptive design", "", sep=table.sep); cat("\n")
...
#     
#     est.fixed = colMeans(matrix(unlist(lapply(fin, est_extract_fixed)), nrow = length(fin), byrow = TRUE))
#     se.fixed = colMeans(matrix(unlist(lapply(fin, se_extract_fixed)), nrow = length(fin), byrow = TRUE))
#     fixed = paste0("$", round(est.fixed, n.digits), "$ $(", round(se.fixed, n.digits), ")$")
#     fixed[11] = ""
#     
#     est.xe = colMeans(matrix(unlist(lapply(fin, est_extract)), nrow = length(fin), byrow = TRUE))
#     se.xe = colMeans(matrix(unlist(lapply(fin, se_extract)), nrow = length(fin), byrow = TRUE))
...
#     fin = readRDS(paste0("results/", prefix, ".RDS"))
#     
#     est.xexei = colMeans(matrix(unlist(lapply(fin, est_extract)), nrow = length(fin), byrow = TRUE))
#     se.xexei = colMeans(matrix(unlist(lapply(fin, se_extract)), nrow = length(fin), byrow = TRUE))
#     xexei = paste0("$", round(est.xexei, n.digits), "$ $(", round(se.xexei, n.digits), ")$")
# 	xexei[11] = ""
#     
#     est.fc = colMeans(matrix(unlist(lapply(fin, est_extract_fc)), nrow = length(fin), byrow = TRUE))
#     se.fc = colMeans(matrix(unlist(lapply(fin, se_extract_fc)), nrow = length(fin), byrow = TRUE))
#     fc = paste0("$", round(est.fc, n.digits), "$ $(", round(se.fc, n.digits), ")$")
#     fc[11] = ""
#     
# 	prefix = paste0("adaptive_sample_size_Xei_", method)
#     fin = readRDS(paste0("results/", prefix, ".RDS"))
#     
#     est.xei = colMeans(matrix(unlist(lapply(fin, est_extract)), nrow = length(fin), byrow = TRUE))
#     se.xei = colMeans(matrix(unlist(lapply(fin, se_extract)), nrow = length(fin), byrow = TRUE))
#     xei = paste0("$", round(est.xei, n.digits), "$ $(", round(se.xei, n.digits), ")$")
#     xei[11] = ""
#     
#     sink(fn.out, append=TRUE)
#     for (i in seq(length(covariate))) {
#         cat(covariate[i], fixed[i], xe[i], xexei[i], xei[i], fc[i], sep=table.sep); cat("\n")
...
#### Triplot ################################################################
png("sum2_triplot.png", units = "in", res = 500, height = 4.4, width = 12)
layout(matrix(c(1, 2, 3, 4, 4, 4), 2, 3, byrow = TRUE), heights = c(4, 0.4))

# png("sum2_triplot.png", units = "in", res = 500, height = 8.8, width = 8)
# layout(matrix(c(1, 2), 2, 1, byrow = TRUE), heights = c(8, 0.8))

fin = readRDS("../20191130/results/adaptive_design_Xe_ACML.RDS")
design.set = matrix(unlist(lapply(fin, design_extract)), ncol = 3, byrow = TRUE)[1:NSIM,]
...
fin = readRDS("../20191130/results/adaptive_design_XeXei_ACML.RDS")
design.set = matrix(unlist(lapply(fin, design_extract)), ncol = 3, byrow = TRUE)[1:NSIM,]
total = rowSums(design.set)
p1 = design.set[,1]/total
p2 = design.set[,2]/total
...
p.fixed = c(10, 120, 20)/150
tripoints(p.fixed, pch = 13)

fin = readRDS("../20191130/results/adaptive_design_Xe_MI.RDS")
design.set = matrix(unlist(lapply(fin, design_extract)), ncol = 3, byrow = TRUE)[1:NSIM,]
total = rowSums(design.set)
p1 = design.set[,1]/total
p2 = design.set[,2]/total
p3 = design.set[,3]/total
triplot(p1, p2, p3, label = c("V = 1", "V = 2", "V = 3"), main = "MI Replicate = 5")

fin = readRDS("../20191130/results/adaptive_design_XeXei_MI.RDS")
design.set = matrix(unlist(lapply(fin, design_extract)), ncol = 3, byrow = TRUE)[1:NSIM,]
total = rowSums(design.set)
p1 = design.set[,1]/total
p2 = design.set[,2]/total
...
p2 = design.set[,2]/total
p3 = design.set[,3]/total
triplot(p1, p2, p3, label = c("V = 1", "V = 2", "V = 3"), main = "MI Replicate = 10")

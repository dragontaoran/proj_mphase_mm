...
gen_XI = function(SEED, NLOW, NHIGH, N, PXE, OMEGAZ.Xe) {
  ...
  for (i in seq(N)) {
	...
	time.tmp = (seq(nclust[i])-1)/(NHIGH-1)*2
...
find_n2_unif = function(N2, MODEL, LVMODEL, TMODEL, Q, EVXEI, ITERLIM, VERBOSE=FALSE, FIT1, DATA, MARG.EXP.FORMULA, 
                        RNGp=c(0.05, 0.95), S1INFO, GRIDREP=1, GRIDINC=20, METHOD='ACML', MIREP=MIREP) {
  time_start = Sys.time()
...
idea = function(iFIT, iDAT, iS1INFO, REMAINING, MARG.EXP.FORMULA, N2FIXED, METHOD, WAVED2, INC, 
                MODEL, LVMODEL, TMODEL, ITERLIM, MIREP, RNGp, VERBOSE, TARGETVAR, TARGET, BREP, Q) {
...
#       best_des = poss_des2[which.max(poss_des2$opt),c("n20", "n21", "n22")]
      ...
      if(VERBOSE) cat('\n\r', date(), '\n\r', paste('Calculating X values for D2[0,X,0]; ', mxx, ' of ', BREP, '\n', sep=''))
      
      best_des_hx = find_n2_unif(N2=WAVED2, MODEL, LVMODEL, TMODEL, Q, EVXEI=TARGETVAR, ITERLIM, VERBOSE, FIT1=iFIT, DATA=tmpXe[[mxx]], 
                                 MARG.EXP.FORMULA, S1INFO=iS1INFO, GRIDREP, GRIDINC, METHOD=METHOD, RNGp=RNGp, MIREP=MIREP)
...
twowave_aODS = function(DATA=NULL, SEEDS=1, YSEED=NULL, NLOW=4, NHIGH=6, N=5000, PXE=0.25,
  ...
  idea_res = idea(iFIT=fit1, iDAT=adat, iS1INFO=s1info, REMAINING=tmp_ss_kn, MARG.EXP.FORMULA=MARG.EXP.FORMULA, N2FIXED=N2FIXED, 
                  ...
                  MIREP=MIREP, TARGETVAR=TARGETVAR, TARGET=TARGET, VERBOSE=VERBOSE, RNGp=RNGp, BREP=BREP, Q=Q)
